...
echo "$($PSQL "SELECT winner.name AS name FROM games INNER JOIN teams winner ON games.winner_id = winner.team_id WHERE round='Eighth-Final' AND year=2014 UNION SELECT opponent.name AS name FROM games INNER JOIN teams opponent ON games.opponent_id = opponent.team_id  WHERE round='Eighth-Final' AND year=2014 ") "
